...
**State:** **25 migrations (`0001`–`0025`).** `npm run typecheck` + `npm run build` green (marketing pages still static-generated). Lint = **12 errors** (baseline; none from this work, do NOT regress). Full E2E suite = **98 pass / 0 reds** (Phase 7: 4 flakes fixed + `pdp-lot-coa` 2 + `rep-support` 7). Org-scoped RLS + approval gate + the full rep/commission engine + the Phase-5 admin power tools + customer impersonation + support tickets + lot/COA-lite + the Phase-6 ops layer (healthz · mock-aware rate-limiting · sidebar nav guards) are LIVE on prod. `product_prices` = 60 SKUs (4× retail); 9 backfilled orgs. `graphify-out/` current (547 nodes). Git clean, pushed; latest prod deploy `a071214` READY on clarivenlabs.com. Supabase advisors WARN-only (definer-RPC functions anon/authenticated-executable by design; the commission engine fns `write_rep_commission_for_order` + the order/lock trigger fns are REVOKEd from client roles → trigger-only, absent from the advisor list; every policy `(select ...)`-wrapped, every fn `search_path`-locked, every FK indexed).
...
**Phase 5 — Admin parity + impersonation + support (STARTED). Slice 1 = admin power tools, COMPLETE (commits `22a350d` → `fb853ff`, migration `0020`, all deployed READY).** Plan: `~/.claude/plans/i-am-going-to-validated-prism.md` § Phase 5. Sam chose admin-power-tools as the Phase-5 entry point.
  ...
  - **(c) lot/COA-lite — COMPLETE** (migration `0023`). Physical-lot tracking + expiration alerting, ops-facing. `product_lots` (keyed `(product_slug, strength_label, lot_number)` like `product_coas` — **no `product_variants`, no qty**; an `active` flag replaces Bioveris's `qty_on_hand>0` gate) + `lot_alert_notifications` (idempotency, port of Purity 042). Both **admin-only RLS** (`(select is_admin())`). Lot COA PDFs reuse the existing public `product-coas` bucket (`lots/…` path). **Daily cron** `/api/cron/lot-expiration` (`0 14 * * *` in `vercel.json`): scans active lots in the 90d horizon, `selectLotsToNotify` (pure helper `src/lib/inventory/lot-expiration-alerts.ts` — 90/60/30/14-day thresholds, 1-day window, per-lot dedupe) → emails every admin, dedupe-row-first. **Email no-ops until Resend creds** (Phase 7), same as order emails. `lot-expiration-warning` template (RUO-clean, via `baseEmailHtml`) + `EMAIL_KINDS`. `src/lib/lots/{constants,queries,actions}` + `/admin/lots` (`LotManager` island: add-lot + per-row COA upload/active-toggle/delete, single parent state, `useState`-stabilised `now`). Admin nav += Lots; audit `product_lot.*` (new target). Adversarial RLS probe (admin reads+writes, customer/anon 0) + lock `lot-expiration.spec.ts` (4/4: pure-helper unit, cron 401/200, idempotency, admin add-lot UI). **Fast-follow DONE (Phase 7, migration `0024`):** lot COAs now surface on the **customer PDP** via the anon `list_public_lot_coas(p_slug)` SECURITY DEFINER RPC (active lots that have a COA, safe projection — no notes/received_at/uploaded_by); `product_lots` base RLS stays admin-only. CI lock `pdp-lot-coa.spec.ts` (2/2 — safe projection + anon base-table denied).
  - **(b) support tickets — COMPLETE** (migration `0022`). Org-scoped customer↔admin ticketing, RUO-clean. `support_tickets` + `ticket_messages` with **effective-user RLS** (rides the 0021 helpers — `tkt_read`/`tmsg_read` scope to `(select user_org_id())` or `(select is_admin())`; internal notes admin-only; an impersonating admin correctly acts AS the customer). Customers never UPDATE the ticket row (admin-only `tkt_upd`); **reopen-on-customer-reply + queue re-sort run in a SECURITY DEFINER trigger** `ticket_reopen_on_reply` (keys on the author's REAL role; EXECUTE revoked from client roles → trigger-only, off the advisor list). Sequential `ticket_number` rendered `SUP-{n}`. No new client RPC. `src/lib/support/{constants,queries,actions}` (+ `schemas/support.ts`, client-safe constants split); `/portal/support[/new,/[id]]` (client `SupportTicketList` chip island — useState/useMemo, no server roundtrip) + `/admin/support[/[id]]` (KPI strip + reply-with-internal-note + triage: status/priority/assign-to-me). Admin triage writes `admin_audit_log` (`support_ticket.*` — new audit target type). Nav += Support (portal + admin). Adversarial RLS probe (8/8) + lock `support-tickets.spec.ts` (5/5: create → triage → internal-note hidden from customer → customer reply reopens → cross-org 404). ~~**`/rep/support` stays the Phase-4 email stub**~~ **WIRED (Phase 7, migration `0025`)** — needed an RLS extension (org-less reps; the "schema already supports it" note was wrong). `support_tickets.organization_id` is now nullable; `tkt_read`/`tkt_ins`/`tmsg_read`/`tmsg_ins` gained a `created_by = auth.uid()` author path + an `is_active_rep()` org-null insert branch. Rep pages reuse the support components (`createRepTicket`, `listRepTickets`, `SupportTicketList mode="rep"`).
  - **(a) customer impersonation — COMPLETE** (commits `8a1af7f` d1 + `a802644` d2, migration `0021`; plan `~/.claude/plans/expressive-discovering-coral.md`). DB-table-driven **effective-user** model (port of Bioveris/Purity `054`): `impersonation_sessions` + `effective_user_id()` = `coalesce(active-session target for auth.uid(), auth.uid())`. The 5 RLS helpers (`is_admin`/`user_org_id`/`user_org_role`/`is_org_admin`/`is_active_rep`) + `prof_read`/`prof_upd` now resolve the effective user, so an admin acts **exactly** as a customer — and `is_admin()` drops to **false** mid-session (admin-only RLS can't leak) — with **no service-role + no JWT hook**. `start_impersonation` (15-min TTL, one-active-per-admin, justification 10–500, real-admin gate via a DIRECT profiles read, self/admin-target guards) + `end_impersonation` + `get_impersonation_context` + `current_impersonated_user_id`; `admin_audit_log.impersonated_user_id` col + BEFORE INSERT auto-fill trigger. **UI:** Impersonate button on `/admin/organizations/[id]` member rows → justification modal → `/portal` + a **global fixed-bottom banner** (client island so the root layout stays static — marketing pages keep static generation) with countdown + End session; `/admin/impersonations` ledger; audit-viewer `'user'` target + "acting as" chip. **The 5 helper rewrites did NOT regress normal access** (`effective_user_id()` ≡ `auth.uid()` with no session; full suite 72 pass / 4 flakes). Locks: `admin-impersonation.spec` (5/5 adversarial: scoping/isolation/is_admin-false/guards/RLS/audit) + `admin-impersonation-ui.spec` (1/1).
  - **Phase 5 COMPLETE.** Optional fast-follows: ~~surface a lot's COA on the customer PDP~~ **DONE** (Phase 7, migration `0024`); ~~wire `/rep/support` to the ticket engine~~ **DONE** (Phase 7, migration `0025`) — the support RLS was broadened (author path + active-rep org-null insert branch; `organization_id` now nullable) so a rep files/reads/replies to org-less tickets behind `is_active_rep()`; customer guarantees intact (rep-support.spec 7/7 + support-tickets.spec 5/5). **Next: Phase 7** (cutover).

**Phase 6 — Ops, perf & launch hardening (COMPLETE 6a–6c, commits `6249576` → `5c96675`, no migration, all deployed READY).** Plan: `~/.claude/plans/i-am-going-to-validated-prism.md` § Phase 6.
  - **6a healthz** `6249576` — `/api/healthz` (GET, public). Supabase liveness via an anon-safe `is_admin()` definer ping + `gbp`/`twilio` (`configured|mock|missing`) + `resend`/`cron` (`configured|missing`). Status booleans/strings ONLY — never a secret/env value. 200 when Supabase up, 503 when not. (Proxy doesn't gate `/api`.) Lock `healthz.spec` (1/1).
  - **6b rate-limiting** `27271b1` — `src/lib/ratelimit.ts` (Upstash sliding-window via `@upstash/ratelimit` + `@upstash/redis`). **Mock-aware:** null limiter when `UPSTASH_REDIS_REST_URL`/`_TOKEN` absent → `checkRateLimit` no-ops (allow); **fails OPEN** on Redis/limiter errors. Enforced on `POST /api/orders` (5/60s per user — order spam fans out to GBP invoices + ops SMS) + the 5 admin writes (`prices`/`affiliate-codes`/`affiliates`/`orders/[id]`/`orders/bulk`, 30/60s per IP, rate-limit-first). `429` + `Retry-After`. Lock `ratelimit.spec` (2/2 no-op proof). `.env.local.example` already has the UPSTASH placeholders — real 429 activates once they're set (Phase 7).
  - **6c perf** `5c96675` — client `NavLink` (`src/components/ui/nav-link.tsx`: `useTransition` pending-guard + `prefetch={false}` + cmd/ctrl/shift/middle-click passthrough + `aria-current` active) replaces plain `<Link>` in admin/portal/rep layouts → kills the Next-16 RSC-stream rapid-click freeze. (No cacheComponents Suspense split — deferred.) Capped `listReps()` (200) + `/admin/affiliates` (affiliates 200 / codes 500 / aggregation filtered to affiliate-attributed orders + 5000 backstop, correctness-preserving). Lock `nav-perf.spec` (1/1).
  - **Deferred (opt-in, rationale in § Up next):** Sentry (needs `SENTRY_DSN` + Turbopack-compat review), `cacheComponents`/PPR (net-negative for a force-dynamic app), `use-chip-filter` port (no consumer surface yet).
...
- ✅ **Resend DONE** — domain `updates.clarivenlabs.com` verified; `RESEND_API_KEY` / `RESEND_FROM_EMAIL` (`ClarivenLabs <noreply@updates.clarivenlabs.com>`) / `RESEND_REPLY_TO` (`support@clarivenlabs.com`) set on Vercel prod+preview+dev + `.env.local`; live test send OK; deploy `7918a7d` READY → prod `/api/healthz` shows `resend:"configured"`. **App transactional email layer is LIVE** (order placed/paid/shipped + lot-expiration alerts). ⚠️ The key was pasted in chat — **rotate it**, then update **5 touchpoints**: 3 Vercel envs + `.env.local` + re-run `configure-supabase-auth.mjs` (the script also stores it as Supabase SMTP `smtp_pass`).
- ✅ **Supabase auth DONE** (2026-06-02) — ran `scripts/configure-supabase-auth.mjs`: Site URL `https://clarivenlabs.com`, redirect allow-list (apex+www+vercel reset/callback), `mailer_autoconfirm=false` (verification ON), Resend SMTP, branded RUO recovery template. **Verified end-to-end**: real `/recover` for sam@ovington.io delivered to inbox from `noreply@updates.clarivenlabs.com`, link `redirect_to=https://clarivenlabs.com/auth/callback?next=/reset-password` (apex, NOT localhost). ⚠️ The `SUPABASE_ACCESS_TOKEN` was pasted in chat — **delete/rotate it** (no longer needed; config is persisted).
...
**Pre-existing test issues — ✅ ALL FIXED in Phase 7 track (b) (commit `f8bbe19`; full suite 89/89):**
- ~~`auth.spec.ts` × 2 + `admin-pricing.spec.ts`: sign-in redirect race~~ **FIXED** — `waitForURL(/\/portal/)` after the server-action sign-in (Set-Cookie settles) before navigating. admin-pricing additionally scopes edits to the exact `(slug, strength)` row via a new `data-sku` hook on PricingEditor rows; the old `Retail.first()` raced the filter re-render and the `/^saved/` wait was satisfied by a never-dirty sibling row (so the DB read could precede the POST).
- ~~`cart-and-order.spec.ts` "expired code is rejected"~~ **FIXED** — seed a cart line first (the affiliate input sits behind `cart.lines.length > 0`) **and** fixed `validateCode`: the RPC returns **0 rows** for an expired/unknown code (not a `valid=false` row), so an empty result now shows "That code isn't active" instead of the misleading transient-error "Could not check this code right now". Genuine RPC `error` keeps the transient message.
- ~~`cart-and-order.spec.ts` "add to cart" 5 mg~~ **FIXED in Phase 2B** — now `/products/single-regulator` + "10 mg" (real seeded price).

**Up next: Phase 7 — stabilize, cutover, handoff** (`~/.claude/plans/i-am-going-to-validated-prism.md` § Phase 7). **Phase 6 ops/perf COMPLETE** (6a healthz · 6b rate-limiting · 6c nav guards + bounded queues). **Deferred from Phase 6 (opt-in):** Sentry (needs `SENTRY_DSN` + a Turbopack-compat review — wire DSN-gated when the cred lands; **cleanest path = install the Vercel Marketplace Sentry integration → auto-provisions `SENTRY_*` env like Upstash. After a ~2-min human account bootstrap, Claude can fully drive the rest: project/DSN via the Sentry API, env, DSN-gated + PII-scrubbed `@sentry/nextjs` (Next-16 instrumentation hook, no webpack plugin), alerts/releases via `sentry-cli`, and live issue triage via the official Sentry MCP**) and `cacheComponents`/PPR (net-negative for a force-dynamic app; skip unless a cacheable surface appears); `use-chip-filter` port (no consumer yet). Phase 7 **track (b) DONE** — the 4 known flakes are stabilized (full suite 89/89; commit `f8bbe19`). Phase 7 **track (a) cutover IN PROGRESS** (2026-06-02): ✅ **Resend app email layer** + ✅ **Supabase branded RUO auth email** (Site URL/allow-list/recovery template, `mailer_autoconfirm=false`) are **LIVE & verified on prod** — see the **§ Cutover creds — status** block above for exactly what's done + verified. **Remaining cutover = Upstash + live GBP/Twilio ONLY** (do NOT redo Resend/Supabase): paste `UPSTASH_REDIS_REST_URL`/`_TOKEN` → set Vercel ×3 → live 429s; paste live GBP/Twilio creds → set Vercel prod → flip `GBP_MOCK`/`TWILIO_MOCK=false` → live order round-trip. Then final `get_advisors` + handoff. Steps in **`docs/phase7-cutover-runbook.md`** §3–6. 🔐 **Two secrets pasted in chat — pending Sam rotation:** Resend key (rotate → 3 Vercel envs + `.env.local` + re-run `configure-supabase-auth.mjs`) + `SUPABASE_ACCESS_TOKEN` (just delete). Advisors: WARN-only, 0 ERROR (optional: enable HIBP leaked-password protection to clear `auth_leaked_password_protection`).

**⚠️ Impersonation footgun for the next session:** `is_admin()` / `user_org_id()` / `is_org_admin()` / `is_active_rep()` are now **effective-user-aware** — while an admin has an active impersonation session they resolve the *customer's* identity (and `is_admin()` returns false). Any NEW gate must use a **direct `profiles` read on `auth.uid()`** (not `is_admin()`) when it must reflect the REAL admin mid-session — see `start_impersonation` + the `impersonation_sessions` RLS for the pattern. `effective_user_id()` and `impersonation_sessions` RLS read tables DIRECTLY to avoid recursion.
...
**New Phase-5 E2E specs (all green; permanent CI locks):** `admin-audit` (render + actor/payload hydrate, target-type filter, CSV, non-admin proxy bounce — 4/4), `admin-orders-tools` (bulk status on selected only, inline shipped + tracking, CSV — 3/3), `admin-sales-dashboard` (KPI cards + non-empty chart, range-chip re-nav + active state, non-admin bounce — 3/3), `admin-impersonation` (API-level adversarial: scoping/isolation/is_admin-false/guards/RLS/audit — 5/5), `admin-impersonation-ui` (Impersonate → modal → banner → End — 1/1), `support-tickets` (round-trip: customer create → admin reply + internal note + resolve → customer sees reply but NOT the internal note → customer reply reopens via trigger → cross-org customer gets 404 — 5/5), `lot-expiration` (pure-helper threshold/window/idempotency unit cases + cron 401/200 auth + seed-lot→cron emits 14-tuple + dedupe-row + re-run idempotent + admin add-lot UI — 4/4). `truncateTestData` org prefixes += `e2e-ordtools-` + `e2e-sales-` + `e2e-imp-` + `e2e-impui-` + `e2e-support-`, and now wipes `ticket_messages` + `support_tickets` + `lot_alert_notifications` + `product_lots` wholesale (net-new tables, no prod rows yet); these specs seed their own data and self-clean.

**New Phase-7 E2E specs (permanent CI locks):** `pdp-lot-coa` (anon `list_public_lot_coas` safe projection + active/has-COA filter + anon base-table denied — 2/2), `rep-support` (rep opens/reads an org-less ticket · can't plant one in a customer org · rep↔rep + customer isolation · admin sees it · rep reply OK but internal-note blocked · foreign-rep reply blocked — 7/7). `truncateTestData` wipes `support_tickets`/`ticket_messages`/`product_lots` wholesale; the specs use synthetic slugs + `e2e-repsup-` users and self-clean.
...
| **`support_tickets`** | `ticket_number` (identity, `SUP-{n}`), `organization_id FK?` (**nullable** since `0025` — null = rep ticket), `created_by FK`, `order_id FK?`, `subject`, `body`, `status` (`open\|in_progress\|waiting_customer\|resolved\|closed`), `priority` (`low\|normal\|high\|critical`), `category?` (`general\|order_issue\|billing\|product_quality\|compliance`), `assigned_to FK?`, `resolved_at` | Phase 5 (b), migration `0022`; extended `0025`. RLS: read = org member OR admin OR **author** (`created_by = auth.uid()`); insert = own-org+self OR **active rep** (org-null+self); **admin-only UPDATE** (`tkt_upd`). `set_updated_at` trigger. |
| **`ticket_messages`** | `ticket_id FK`, `author_id FK`, `body`, `is_internal` (admin-only notes), `created_at` | Phase 5 (b). RLS: visible if parent ticket visible AND (`is_internal=false` OR `is_admin()`); insert authored-by-self + same gate. **`ticket_reopen_on_reply` AFTER-INSERT trigger** (SECURITY DEFINER, EXECUTE revoked → trigger-only) reopens a resolved/waiting ticket on a non-staff non-internal reply + touches the parent so the queue re-sorts. |
| **`product_lots`** | `product_slug`, `strength_label`, `lot_number` (unique together), `expiration_date`, `coa_file_path`/`_name`/`_bytes`/`_uploaded_at?`, `received_at?`, `notes?`, `active`, `uploaded_by` | Phase 5 (c), migration `0023`. Physical lots + per-lot COA + expiration. **No qty/inventory** (Clariven has no stock). RLS `lots_admin_all` = admin-only. COA files in the public `product-coas` bucket (`lots/…`). Customer PDP reads active-lot COAs via the anon `list_public_lot_coas` RPC (migration `0024`) — base table stays admin-only. |
| **`lot_alert_notifications`** | `lot_id FK`, `threshold_days` (90\|60\|30\|14), `sent_at`, unique `(lot_id, threshold_days)` | Phase 5 (c). Idempotency for the lot-expiration cron (port of Purity 042) — one row = "already alerted this lot at this threshold". Admin-only RLS; cron writes via service-role (bypasses RLS). |
...
| `list_public_lot_coas(p_slug text)` | text | `(lot_number, strength_label, expiration_date, coa_file_path, coa_file_name)` | Phase 7, migration `0024`. **anon-callable**; active lots that have a COA, safe projection (no notes/received_at/uploaded_by/ids). Customer PDP lot-COA boundary — `product_lots` base stays admin-only. |
...
- `/portal/support` — **NEW (Phase 5 b)** — org-scoped tickets. List (client chip filter: All/Active/Resolved) + `/new` (subject, category, optional order link) + `/[id]` (thread + reply; replying to a resolved ticket reopens it).
...
- `/admin/support` — **NEW (Phase 5 b)** — global ticket queue: KPI strip (Open / In progress / Resolved 7d / Oldest open) + client chip filter; `/[id]` = thread (incl. internal notes) + reply-with-internal-note + triage controls (status / priority / assign-to-me). Audited via `support_ticket.*`.
...
- `/admin/lots` — **NEW (Phase 5 c)** — physical-lot tracking: add lot (product/strength/lot#/expiration/optional COA), per-row COA upload-replace, active toggle, delete. Expiry badges (expired/expiring-soon/current). Feeds the daily lot-expiration alert cron.
...
- `/api/cron/lot-expiration` (daily 14:00 UTC) — **NEW (Phase 5 c)** — alerts admins for active lots crossing 90/60/30/14-day expiration thresholds (idempotent via `lot_alert_notifications`). Email no-ops until Resend creds land.
- `/api/healthz` (GET, public) — **NEW (Phase 6a)** — Supabase liveness + gbp/twilio/resend/cron status (no secrets). 200 up / 503 down.
- **Rate limiting (Phase 6b)** — mock-aware Upstash limiter (`src/lib/ratelimit.ts`) on `POST /api/orders` (5/60s) + admin writes (30/60s); no-ops until `UPSTASH_*` env lands.
- **Sidebar nav rapid-click guard (Phase 6c)** — client `NavLink` (useTransition + `prefetch={false}`) on admin/portal/rep nav; unbounded admin queues capped.
