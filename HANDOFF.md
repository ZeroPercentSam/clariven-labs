...
**Up next: Phase 7 — stabilize, cutover, handoff** (`~/.claude/plans/i-am-going-to-validated-prism.md` § Phase 7). **Phase 6 ops/perf COMPLETE** (6a healthz · 6b rate-limiting · 6c nav guards + bounded queues). **Deferred from Phase 6 (opt-in):** Sentry (needs `SENTRY_DSN` + a Turbopack-compat review — wire DSN-gated when the cred lands) and `cacheComponents`/PPR (net-negative for a force-dynamic app; skip unless a cacheable surface appears); `use-chip-filter` port (no consumer yet). Phase 7 **track (b) DONE** — the 4 known flakes are stabilized (full suite 89/89; commit `f8bbe19`). Phase 7 **remaining = track (a), cred-gated only**: prod cutover per **`docs/phase7-cutover-runbook.md`** (one-shot, idempotent) — Resend DNS → `configure-supabase-auth.mjs` → set live Resend/Upstash/GBP/Twilio env on Vercel ×3 → flip `GBP_MOCK`/`TWILIO_MOCK=false` → redeploy → live order + password-reset round-trip on apex → final advisors. Advisors precheck 2026-06-01: WARN-only, 0 ERROR (one optional hardening: enable HIBP leaked-password protection to clear the `auth_leaked_password_protection` WARN).
